...
sensor_ SensorWarn = {
    .DriverFire = 300,
    .DriverHumidity = 50,
    .DriverInfraredBeam = 1,
    .DriverSmoke = 1400,
    .DriverTemptrue = 19
};

ctrl_driver_ CtrlDriver = {
    .Driver_AudioPlay = 0,
    .Driver_Buzzer = 1,
    .Driver_Led = 0,
    .Driver_Motor = 0,
...
uint16_t ADC_ReadMQ2Sensor(void){
	...
	/*设置ADC时钟*/
	RCC_ADCCLKConfig(RCC_PCLK2_Div2);						//选择时钟6分频，ADCCLK = 8MHz / 2 = 4MHz
	...
	return (uint16_t)(ADC_GetConversionValue(ADC1) / 0.4096);	
}
...
uint8_t ReadInfraredRangingSensor(void){
	...
	return (uint8_t)(!GPIO_ReadInputDataBit(GPIOD, GPIO_Pin_13));
}