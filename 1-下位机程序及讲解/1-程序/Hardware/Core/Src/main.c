...
int main(void)
{
  ...
  OLED_ShowString(0, 0, "OLED init!", OLED_8X16);
  OLED_Update();
  // DHT11_Init();
  ...
  OLED_Update();
  OLED_ShowString(0, 16, "MQ9 Calibrate!", OLED_8X16);
  Calibrate_MQ135();
  printf("MQ135 has Calibrate!\r\n");
  OLED_ShowString(0, 32, "OLED init!", OLED_8X16);
  OLED_Update();
  /* USER CODE END 2 */
  ...
  OLED_ShowString(0, 48, "USART1 init!", OLED_8X16);
  OLED_Update();  
  ...
  OLED_ShowString(0, 0, "USART3 init!", OLED_8X16);
  OLED_Update();    
  
  HAL_ADC_Start_IT(&hadc1);
  printf("ADC has init!\r\n");
  OLED_ShowString(0, 16, "ADC init!", OLED_8X16);
  OLED_Update();   
  ESP01S_Init(&huart4);
  printf("ESP-01S has init!\r\n");
  OLED_ShowString(0, 32, "ESP-01S init!", OLED_8X16);
  OLED_Update();     
  // ÉèÖÃÊý¾Ý½ÓÊÕ»Øµ÷
  ...
  // HAL_TIM_Base_Start(&htim1);
    OLED_ShowString(0, 48, "ESP-01S .....", OLED_8X16);
    OLED_Update();  
    if (ESP01S_Test() == ESP01S_OK)
    {
        printf("ESP-01S Ready!\r\n");
        OLED_ShowString(0, 0, "ESP-01S Ready!", OLED_8X16);
        OLED_Update();         
    }
    OLED_ShowString(0, 16, "ESP-01S Link ...", OLED_8X16);
    OLED_Update(); 
    if (ESP01S_ConnectAP("3BC4E533", "12345678") == ESP01S_OK)
    {
        printf("ESP-01S Connect Wi-Fi!\r\n");
        OLED_ShowString(0, 32, "ESP-01S Link WiFi", OLED_8X16);
        OLED_Update();         
    }        
    OLED_ShowString(0, 48, "ESP-01S Link UDP...", OLED_8X16);
    OLED_Update(); 
    switch (ESP01S_StartUDP("192.168.137.1", 8080, 8080, UDP_MODE_BROADCAST, 0))
    {
      case ESP01S_OK:
          printf("ESP-01S Start UDP!\r\n");
          OLED_ShowString(0, 0, "ESP-01S Link UDP OK", OLED_8X16);
          OLED_Update();           
        break;
      case ESP01S_ERROR:
          printf("ESP-01S Start UDP ERROR!\r\n");
          OLED_ShowString(0, 0, "ESP-01S Link UDP ERROR", OLED_8X16);
          OLED_Update();           
        break;
      case ESP01S_TIMEOUT:
          printf("ESP-01S Start UDP TIMEOUT!\r\n");
          OLED_ShowString(0, 0, "ESP-01S Link UDP TIMEOUT", OLED_8X16);
          OLED_Update();           
        break;
      case ESP01S_BUSY:
          printf("ESP-01S Start UDP BUSY!\r\n");
          OLED_ShowString(0, 0, "ESP-01S Link UDP BUSY", OLED_8X16);
          OLED_Update();           
        break;                        
  ...
  while (1)
  {
    ...
    if(DisplayRunFlag){
      ...
      OLED_ShowString(64, 48, "RUN=", OLED_8X16);
      OLED_ShowNum(96, 48, System_Run_Flag++, 4, OLED_8X16);           
      OLED_Update();
      OLED_Clear();
      // DisplayRunFlag = 0;
    }
